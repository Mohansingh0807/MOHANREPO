package com.example.appjobappli;

import android.content.Intent;
import android.os.Bundle;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;

import androidx.activity.EdgeToEdge;
import androidx.appcompat.app.AppCompatActivity;
import androidx.core.graphics.Insets;
import androidx.core.view.ViewCompat;
import androidx.core.view.WindowInsetsCompat;

public class MainActivity extends AppCompatActivity {
    EditText applicant,qualification,experience;
    Button applynow;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        EdgeToEdge.enable(this);
        setContentView(R.layout.activity_main);
        applicant=findViewById(R.id.applicant);
        applynow=findViewById( R.id.applynow);
        qualification=findViewById(R.id.qualification);
        experience=findViewById(R.id.experience);



        applynow.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String name= applicant.getText().toString();
                String quali= qualification.getText().toString();
                String exper= experience.getText().toString();


                Intent i=new Intent(MainActivity.this,SecondActivity.class);
                i.putExtra("Applicant Number",name);
                i.putExtra("Qualification",quali);
                i.putExtra("Experience",exper);
                startActivity(i);





            }

        });
    }
}